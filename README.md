...
この JSON は構造が素直ではない。**当日と週間で気温の入り方が違い**、週間ブロックは
翌日から始まって当日の枠は空文字になる。さらに `timeDefines` と `temps` の対応は
発表時刻で変わる（朝の発表だと先頭が 09:00 の実況になる）。添字を決め打ちにすると
気温がずれるので、日付が一致する要素だけを集めて最大・最小を取っている。
...
- [x] **1. 土台** — ランチャー化、大時計、日付、診断オーバーレイ、端末セットアップ
  - release APK 1.4MB、ユニットテスト 25 件、実機で動作確認済み
