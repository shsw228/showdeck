...
時計だけは例外で、**画面高**から決める（`CLOCK_HEIGHT_RATIO`）。
この端末の主役で、可能な限り大きく出したいため。

**幅は決めない。** ここは二度間違えた場所なので経緯を残す。

1. 時計を `weight(1.7f)` の箱に入れ、そこに収まるよう「`HH:mm` は文字サイズの
   2.65 倍の幅」と係数を決め打ちした → 実測は 2.34 で、時計の右に 50dp の死んだ空間。
2. 係数を実機のスクリーンショットから測り直した → 数字は合うが、フォントや書式を
   変えたら壊れる。
3. `TextMeasurer` で実測させた → 壊れなくはなったが、まだ**自分で作った隙間を
   自分で塞いでいる**だけ。

正解は 3 つ目ですらなく、**箱を作らないこと**。時計は自然な幅のまま `Row` に渡し、
区切り線と情報レールはその後ろに `Row` が並べる。幅の差分が発生しないので、
埋める計算も要らない。レイアウトにやらせられることを自分で計算し始めたら、
だいたい構造のほうが間違っている。
...
**今日か明日のときだけ 1 ページ目に一行で出す。** ごみは朝に一目で見えないと
意味がなく、スワイプの奥にあると見ようと思ったときにしか見えない。
かといって常時出すと情報が増えて時計が読みにくくなる。

3 ページ目は「次の不燃ごみはいつか」を確かめる用。そういう場面は稀なので、
スワイプの奥で構わない。

1 ページ目は日付・天気・ごみを **1 つの塊として縦中央**に置く。端に散らして
高さを埋めようとしたら、日付が区切り線の上に、ごみが線の下にはみ出して、
線が何も区切らなくなった。出すものが無い日は場所ごと空ける
（空の枠を残すと、何かが出るはずの場所に見えて落ち着かない）。
