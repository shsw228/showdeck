...
| 長押し | 操作パネル（明るさ、消灯、アラーム、ポモドーロ、権限の状態、設定画面の URL と資格情報） |
...
作業と休憩を自動で往復する。既定は 25 分作業 / 5 分休憩、4 回ごとに 15 分の長い休憩。
長さは Web 設定画面から変えられる。

- 区間が終わるたびに鳴り、**次の区間へ自動で進む**
- 発報を止めてもポモドーロは止まらない。切り替わりのたびに止まっては用を成さない。
  終わらせるのは操作パネルか Web の「終了」
- 動作中は時計の下に `作業 1  24:53` と出る。部屋の向こうから動いていると分かればよいので 1 行に留める
...
- [x] **1. 土台** — ランチャー化、大時計、日付、診断オーバーレイ、端末セットアップ
  - release APK 1.5MB、ユニットテスト 38 件、実機で動作確認済み
