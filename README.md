...
タイマーの開始、夜間モードの時間帯、バックライトの値、消灯の設定、天気の地域コード、
...
気象庁の予報 JSON を使う。API キーが不要で、国内の精度も十分。

```
https://www.jma.go.jp/bosai/forecast/data/forecast/{地域コード}.json
```

表示するのはアイコン・最高最低・降水確率の 3 つだけ。5.5 インチを 3m から見て
読めるのはその程度で、風速や週間予報を載せても誰も読まない。気象庁の文言
（「雨　夕方　まで　時々　くもり」）は Web 設定画面でだけ見せる。

### 気温の扱い

この JSON は構造が素直ではなく、気温は二段構えで間違えやすい。

**対応は時刻の枠で決まる。`00:00` が最低、`09:00` が最高。**

```
2026-08-11T09:00 -> 29   今日の最高
2026-08-11T00:00 -> 29   今日の最低
2026-08-12T00:00 -> 22   明日の最低
2026-08-12T09:00 -> 28   明日の最高
```

並び順は日によって入れ替わる（当日だけ 09:00 が先に来る）ので、添字でも
「その日の最大・最小」でもなく時刻で突き合わせる。

さらに、**発表時刻を過ぎた当日の枠は実況値で埋まり、最高／最低の意味を失う。**
上の例では当日が 29／29 になっていて、そのまま出しても何の情報にもならない。
当日が潰れている日は明日の予報に切り替え、画面に「明日」と添えている。

週間ブロックは翌日から始まり当日の枠は空文字なので、当日の気温には使えない。

アイコンは画像を持たず Canvas で描く。ビットマップだと夜間の赤単色パレットに
追従できず、そこだけ白く浮くため。線画ではなく塗りにしているのは、円を重ねた
線画の雲が実機で団子にしか見えなかったから。
...
- [x] **1. 土台** — ランチャー化、大時計、日付、診断オーバーレイ、端末セットアップ
  - release APK 1.4MB、ユニットテスト 26 件、実機で動作確認済み
...
├── weather/
│   ├── Weather.kt           気象庁 JSON の解析（純粋関数・テスト対象）
